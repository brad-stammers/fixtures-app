...
class AppBuilder {
    ...
    get resolvableExtensionsPattern() {
        return paths_1.extensionsPattern(this.adapter.resolvableExtensions());
    }
    ...
    impliedAddonAssets(type) {
        let result = [];
        for (let addon of sortBy_1.default(this.adapter.allActiveAddons, this.scriptPriority.bind(this))) {
    ...
    adjustImportsPlugin(engines) {
        ...
        for (let { destPath, appFiles } of engines) {
            for (let [relativePath, originalPath] of appFiles.relocatedFiles) {
                relocatedFiles[path_1.join(destPath, relativePath)
                    .split(path_1.sep)
                    .join('/')] = originalPath;
            }
        }
    ...
    templateCompiler(config) {
        ...
        let { plugins: macroPlugins, setConfig } = macros_1.MacrosConfig.astPlugins();
    ...
    testJSEntrypoint(engines, prepared) {
        ...
        // module dependency.
        testModules.unshift(paths_1.explicitRelative(path_1.dirname(myName), this.topAppJSAsset(engines, prepared).relativePath));
        let amdModules = [];
        // this is a backward-compatibility feature: addons can force inclusion of
        ...
        for (let addon of engine.addons) {
            ...
            if (implicitModules) {
                ...
                for (let name of implicitModules) {
                    let runtime = path_1.join(addon.name, name).replace(this.resolvableExtensionsPattern, '');
